                  ...
                  ...)                      # passed to geom_text for node labels
{
  ...
  plotcord <- do.call(placement, list(m, list( area=(nrow(m)*2)^2 )))
  ...
  plotcord$class <- network::get.vertex.attribute(net, "class")
  plotcord$uniprot <- network::get.vertex.attribute(net, "uniprot")
  
  # plot the network
  pnet <- ggplot(plotcord, aes(X1, X2)) +
    geom_segment(aes(x = X1, y = Y1, xend = X2, yend = Y2,size=width), 
  ...
  plotcord$shape <- sapply(plotcord$class,function(cl) { if (cl == "Protein") { return (16) } else if (cl == "Complex") { return (16) } else if (cl == "BiochemicalReaction") { return(0) } else { return (32) } })
  ...
  if(length(unique(plotcord$id)) > 1 | unique(plotcord$id)[1] != "")
    pnet <- pnet + geom_text(aes(label = uniprot), data=subset(plotcord,class %in% c("Protein","Complex")), size = 1 + sqrt(size), ...)
